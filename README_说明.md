...
### B. 核心选股公式 (Selection Logic)

系统对全市场 5000+ 只股票进行实时扫描，通过 **加权评分算法 (Weighted Scoring)** 决出胜负：

$$ Score = Amount \times (1 + MomentumBoost + VolumeBoost) $$

1.  **基础分 (Base)**：
    *   **成交额 (Amount)**：真金白银是唯一的选票。
2.  **动量加成 (Momentum Boost)**：
    *   涨幅 > **8.5%** (准涨停)：权重 **x 3.0** (确保打板票置顶)
    *   涨幅 > **5.0%** (大阳线)：权重 **x 1.5**
3.  **量能加成 (Volume Boost)**：
    *   量比 > **2.0** (倍量)：权重 **x 1.2**
    *   *注：量能对比基于历史数据 (History Data)，而非随机模拟。*
4.  **Top 30 截断**：
    *   四大赛道各取分数最高的 30 名。

> **结果**：既保证了**大成交额**的中军龙头在榜，又让**缩量加速**的连板妖股能瞬间冲到第一名，完美契合短线战法。

### C. 市场热度算法 (Heat Algorithm)

为了量化监控“变盘”，我们定义了**“热度”**指标：

*   **定义**：每分钟内，排行榜发生“新陈代谢”的剧烈程度。
*   **计算公式**：
    $$ Heat = Count(Unique(New\_Entry)) $$
    *   `New_Entry`: 当一只股票的成交额超过榜单第 30 名，强行挤掉原第 30 名并上榜，视为一次“新进”。
    *   `Unique`: 使用集合 (Set) 去重。同一分钟内，某股票反复进出，仅记 **1** 点热度。
*   **状态阈值**：
    *   `0-10`: **情绪稳定** (适合持股)
    *   `10-30`: **正常轮动** (关注新目标)
    *   `>30`: **剧烈变盘** (高风偏博弈)
