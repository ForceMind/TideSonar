...
2.  **Top 30 截断**：每个赛道只取前 30 名，四大赛道共 120 只。这意味着能上榜的，每一个都是该板块当下的**资金“龙一”到“龙三十”**。
3.  **实时动态更新**：如果一只股票资金掉队了，它会立刻从榜单消失，被新的更强的股票取代。

### C. 热度计算逻辑 (Churn Rate)
为了量化市场的**“轮动速度”**，我们引入了“热度”指标：
*   **触发条件**：当 Top 30 榜单已满员，此时有一只新股票的成交额超过了榜单第 30 名，将其挤出榜单。
*   **去重统计**：系统使用 `Set` 集合对每分钟内新上榜的股票代码进行去重。
    *   例如：股票 A 在 1 分钟内反复进出榜单 10 次，热度只 +1。
*   **计数重置**：每分钟结束后，热度计数器归零，生成当日分钟级热度曲线。
